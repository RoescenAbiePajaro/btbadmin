...
// ==================== LOGIN COUNTS BY ROLE ====================
router.get('/login-counts-by-role', requireAdmin, async (req, res) => {
  try {
    const { period = '30d', startDate, endDate } = req.query;
    ...
    // Get login counts grouped by role
    const loginCounts = await Click.aggregate([
      {
        $match: {
          type: 'login',
          location: 'login_success',
    ...
    // Ensure all roles are present in response
    const roleMap = {
      student: 0,
      educator: 0,
      admin: 0
    ...
    loginCounts.forEach(item => {
      if (roleMap.hasOwnProperty(item._id)) {
        roleMap[item._id] = item.count;
      }
    });
    
    const totalLogins = Object.values(roleMap).reduce((sum, count) => sum + count, 0);
    ...
    res.json({
      ...
      data: [
        { name: 'Student', value: roleMap.student, color: '#0088FE' },
        { name: 'Educator', value: roleMap.educator, color: '#F472B6' },
        { name: 'Admin', value: roleMap.admin, color: '#8B5CF6' }
      ],
      total: totalLogins,
      breakdown: roleMap,
      timestamp: new Date().toISOString()
    ...
    console.error('Login counts by role error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to fetch login counts by role'
    });
  }
});
