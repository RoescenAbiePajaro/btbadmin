...
// ==================== CLASS STATUS ANALYTICS ====================
router.get('/class-status', requireAdmin, async (req, res) => {
  try {
    const { period = '30d', startDate, endDate } = req.query;
    ...
    // Format status trends for line chart
    const statusTrendsData = statusTrends.map(trend => ({
      date: trend._id,
      active: trend.active,
    ...
    // Format status by school for bar chart
    const statusBySchoolData = statusBySchool.map(school => ({
      name: school._id || 'Unknown',
    ...
    const formattedData = {
      ...
      statusTrends: {
        ...
        lines: [
          { key: 'active', name: 'Active Classes', color: '#10B981' },
          { key: 'inactive', name: 'Inactive Classes', color: '#EF4444' }
        ]
      },
      statusBySchool: {
        data: statusBySchoolData,
        groups: [
    ...
    console.error('Class status analytics error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to fetch class status analytics',
      details: error.message
